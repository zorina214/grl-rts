# Decoupling Hydroclimatic Controls on Displacement of Retrogressive Thaw Slumps in the Qinghai–Tibet Plateau

Code and supplementary materials for the GRL paper:  
**“Decoupling Hydroclimatic Controls on Displacement of Retrogressive Thaw Slumps in the Qinghai–Tibet Plateau”**
...
- Figure generation
- Supplementary datasets used in the study
...
- Compute annual mean LST for each RTS polygon. For each daily LST raster, the script:
  ...
  - Computes annual means by averaging valid daily values per year
...
**Outputs**
- For each RTS ID: `<ID>_LST_mean.txt`
...
- Compute annual precipitation sums for each RTS polygon. For each daily precipitation raster, the script:
  ...
  - Computes annual sums by summing valid daily values per year
...
**Outputs**
- For each RTS ID: `<ID>_pre_sum.txt`
...
### 7) `Elevation.py`
**Purpose**
- Analyze elevation distribution of RTS polygons relative to a DEM.
...
### 8) `Slope.py`
**Purpose**
- Analyze slope distribution of RTS polygons relative to a slope raster.
...
**Purpose**
- Prepare input data by merging annual mean LST, annual precipitation sum, and deformation trend for each RTS.

**Key I/O**  
**Inputs**
- `<ID>_LST_mean.txt` (from `LST_mean.py`)
- `<ID>_pre_sum.txt` (from `Pre_sum.py`)
- `<ID>_UD_pt_with_amplitude_trend.txt` (from UD decomposition)
...
### 12) `Kendall.py`
**Purpose**
- Perform conditional Kendall’s tau analysis on the merged data and generate diagnostic plots.
...
**Workflow**
- Concatenate all RTS records into one DataFrame
...
In addition to the scripts above, we provide supplementary figures for **all 1,034 RTSs** and their **external buffers**:

- Time-series plots for each RTS and its 1–2 km external buffer: `RTSs_1-2kmbuffer_ts_fig(ID)`
- Time series plots for each RTS with error bars: `RTSs_error_bar_fig(ID)`
