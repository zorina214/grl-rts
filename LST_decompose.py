...
"""
Seasonal and trend decomposition of polygon-based LST time series.

For each RTS polygon:
1. Read LST rasters (GeoTIFF stack).
2. For each raster, mask by the RTS polygon and compute the mean of
   all intersecting pixels (polygon mean value).
3. Build a daily time series (date, mean LST) for the polygon.
4. Fit a seasonal + linear trend model:
       y(t) = A * sin(w t + phase) + trend * t + offset
5. Check residuals:
   - mean(|residual|) < 1 (in the LST unit, e.g., °C)
   - residuals behave like white noise (ACF within CI for lags 15–90)
6. If the residual test is passed, extract peak and trough in the
...
# ----------------------------------------------------------------
# LST_RASTER_FOLDER:
#   Folder containing LST rasters.
#   Required format: multiple single-band GeoTIFF (.tif) files.
#   Each file represents one day and the date must be encoded
#   in the file name as YYYYMMDD (e.g., "20160115_LST.tif").
LST_RASTER_FOLDER = r"F:\zyz\DiplomaProject\data\LST_tifs"
...
OUTPUT_FOLDER = r"F:\zyz\DiplomaProject\scripts\revised1_fig\LST_results"
...
def is_white_noise(residuals: np.ndarray, alpha: float = 0.05) -> bool:
    ...
    Only lags 15–90 are checked; all ACF values in that range must be
...
def parse_date_from_filename(filename: str) -> datetime:
    ...
    as YYYYMMDD. Example: "20170115_LST.tif" -> 2017-01-15.
    """
...
def list_lst_rasters(folder: str) -> List[str]:
    """
    List and sort LST raster files in a folder.
...
def plot_series_and_components(
    ...
    """Plot original LST, fitted curve, trend, and residuals."""
    dates_dt = [datetime.fromordinal(d) for d in dates_ord_subset]

    fig, axes = plt.subplots(3, 1, figsize=(12, 8), sharex=True)

    # Original and fitted
    axes[0].plot(dates_dt, values_subset, label="Original LST")
    axes[0].plot(dates_dt, fitted_values, label="Fitted LST")
...
def process_time_series(
    ...
    """
    Process a single polygon LST time series:
    - use daily data as-is (no interpolation)
    ...
    # Time axis relative to the first observation
    t_all = dates_ord - dates_ord[0]
    ...
    for start_date, end_date, internal_start, internal_end in tqdm(
        ...
        dates_subset_ord = dates_ord[mask]
        t_subset = t_all[mask]
        values_subset = values_arr[mask]
        ...
        try:
            ...
            # Residual tests
            # Mean close to zero: threshold = 1 (LST unit)
...
def main() -> None:
    # List LST rasters and parse dates from file names
    lst_files = list_lst_rasters(LST_RASTER_FOLDER)
    lst_dates = [parse_date_from_filename(f) for f in lst_files]

    # Read CRS from first raster and align RTS polygons to it
    first_raster_path = os.path.join(LST_RASTER_FOLDER, lst_files[0])
    ...
    for idx, row in tqdm(
        ...
        # Build polygon time series by masking each LST raster
        for fname, obs_date in zip(lst_files, lst_dates):
            raster_path = os.path.join(LST_RASTER_FOLDER, fname)
        ...
        if len(ts_dates) < 4:
            print(f"{region_id}: fewer than 4 valid LST observations across rasters, skip.")
            continue
        ...
        if peak_dates:
            output_file = os.path.join(
                OUTPUT_FOLDER,
                f"{region_id}_LST_pt_with_amplitude_trend.txt",
            )
